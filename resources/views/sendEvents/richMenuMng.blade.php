...
<body>
    ...
    <p>デフォルトにしたいリッチメニューを選択し、送信してください</p>
    <p>※赤字の物は現在のデフォルトです</p>
                    ...
                    @foreach ($rmList as $rm)
                        {{-- エイリアスIDでまとめる方法：aだけ取得する→aと一致するもの取得 --}}
                            ...
                            <tr style="color:red">
                                    ...
                                    <td>☆</td>
                                    <td align="center"><img src="{{ secure_asset('img/' . $rm->img) }}" alt="img" width="10rem" >
                                    <td>{{ $rm->name }}</td>
                                    <td>{{ $rm->chat_bar }}</td>
                                <td>{{ $rm->is_default }}</td>
                            ...
                            <tr>
                                ...
                                <td align="center"><img src="{{ secure_asset('img/' . $rm->img) }}" alt="img" width="30rem" >
                                </td>
                                <td>{{ $rm->name }}</td>
                                <td>{{ $rm->chat_bar }}</td>
                                <td>{{ $rm->is_default }}</td>
